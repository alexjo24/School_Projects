...
%         x_e_tilde(i,:)=sum_top/sum_bot;
  ...
  %  for j=1:nelm
        ...
        for z= 1:nelm
           %Compute dgdrhotilde
           dgdrhotilde=getdgdrhotilde_el(exC(z,:),eyC(z,:),ep,D,ed(z,:),ed_rho(z,:),p);
            
         %  dg_drho = dg_drho + dgdrhotilde'*drhoTilde_drho_cell{z}; 
            
            indx = enod(z,2:end);
            feee(indx) = feee(indx)+dgdrhotilde;

        end
       

     %   be(j,:) = (1/alpha)*dg_drho*(x_e_tilde(j,:)^(1+alpha));
        
        keyboard
    %end
    be = (1/alpha).*feee'*drhoTilde_drho*(x_e_tilde.^(1+alpha));
